...
new_generate_leave = '''function generateAdminLeaveReport(db) {
    ...
    let filtered = allLeaves.filter(req => {
        const d = new Date(req.appliedOn || req.fromDate || req.date);
    ...
    if (filtered.length === 0) {
        ...
        filtered.forEach(req => {
            ...
            htmlRequests += `
                <tr>
                    <td>${req.appliedOn || req.date || req.fromDate || '-'}</td>
                    ...
                    <td>${req.fromDate || req.date || '-'}</td>
                    <td>${req.toDate || req.date || '-'}</td>
    ...
    filtered.forEach(req => {
        if (req.status === 'Approved' && empStats[req.employeeId]) {
            ...
            if (lType === 'Casual Leave') { st.casual += (req.days || 1); st.casualBal -= (req.days || 1); }
            else if (lType === 'Medical Leave') { st.medical += (req.days || 1); st.medicalBal -= (req.days || 1); }
            else if (lType === 'Annual Leave') { st.annual += (req.days || 1); st.annualBal -= (req.days || 1); }
...
content = re.sub(
    r"function generateAdminLeaveReport\(db\) \{[\s\S]*?(?=function generateAdminPayrollReport\(db\) \{)",
    new_generate_leave + "\n\n",
    content
)

if "function resetLeaveSummaryFilters" not in content:
    content += '''
...
window.openEmployeeLeaveModal = function(empId) {
    ...
    reqs.forEach(r => {
        ...
        if (r.status === 'Approved') {
            if (lType === 'Casual Leave') casual += (r.days||1);
            else if (lType === 'Medical Leave') medical += (r.days||1);
            else if (lType === 'Annual Leave') annual += (r.days||1);
        ...
        historyHtml += `<tr>
            <td>${lType || '-'}</td>
            <td>${r.fromDate || r.date || '-'}</td>
            <td>${r.toDate || r.date || '-'}</td>
            <td>${r.days||1}</td>
