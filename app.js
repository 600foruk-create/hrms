...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('.btn-sub-tab').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (subtab === 'cards') {
                if (typeof window.renderEmployeeCardsTab === 'function') {
                    window.renderEmployeeCardsTab();
                }
            }
        });
    });
...
window.openIdCardTemplatesModal = function() {
    const db = getDb();
    const cp = db.companyProfile || {};
    
    const frontPreview = document.getElementById('id-front-preview');
    const frontMsg = document.getElementById('id-front-msg');
    if (cp.idCardFrontBase64) {
        frontPreview.src = cp.idCardFrontBase64;
        frontPreview.style.display = 'block';
        frontMsg.textContent = 'Change Front Design';
    } else {
        frontPreview.src = '';
        frontPreview.style.display = 'none';
        frontMsg.textContent = 'Click to Upload Front Design';
    }

    const backPreview = document.getElementById('id-back-preview');
    const backMsg = document.getElementById('id-back-msg');
    if (cp.idCardBackBase64) {
        backPreview.src = cp.idCardBackBase64;
        backPreview.style.display = 'block';
        backMsg.textContent = 'Change Back Design';
    } else {
        backPreview.src = '';
        backPreview.style.display = 'none';
        backMsg.textContent = 'Click to Upload Back Design';
    }

    // Reset temporary variables
    window.tempIdFront = cp.idCardFrontBase64 || null;
    window.tempIdBack = cp.idCardBackBase64 || null;

    openModal('modal-id-templates');
};

window.handleIdTemplateUpload = function(event, side) {
    ...
    reader.onload = function(e) {
        ...
        if (side === 'front') {
            window.tempIdFront = base64;
            const preview = document.getElementById('id-front-preview');
            preview.src = base64;
            preview.style.display = 'block';
            document.getElementById('id-front-msg').textContent = 'Change Front Design';
        } else {
            window.tempIdBack = base64;
            const preview = document.getElementById('id-back-preview');
            preview.src = base64;
            preview.style.display = 'block';
            document.getElementById('id-back-msg').textContent = 'Change Back Design';
        }
    };
    reader.readAsDataURL(file);
};

window.saveIdTemplates = function() {
    const db = getDb();
    if (!db.companyProfile) db.companyProfile = {};
    
    db.companyProfile.idCardFrontBase64 = window.tempIdFront;
    db.companyProfile.idCardBackBase64 = window.tempIdBack;
    
    saveDb(db);
    showToast("Templates Saved", "ID Card templates updated successfully.");
    closeModal('modal-id-templates');
    if (typeof window.renderEmployeeCardsTab === 'function') {
        window.renderEmployeeCardsTab();
    }
};

window.generateIdCardHTML = function(user, side) {
    const db = getDb();
    const cp = db.companyProfile || {};
    
    if (side === 'front') {
        const bg = cp.idCardFrontBase64 ? `background-image: url('${cp.idCardFrontBase64}');` : '';
        const profilePicUrl = user.profilePic ? user.profilePic : 'data:image/svg+xml;base64,';
        
        return `
            <div class="id-card-wrapper" style="${bg}">
                <img src="${profilePicUrl}" class="id-photo" alt="Profile">
                <div class="id-name">${user.name}</div>
                <div class="id-desig">${user.designation || user.role}</div>
                <div class="id-idno">ID No: ${user.displayId || user.id.slice(0,8).toUpperCase()}</div>
                <div class="id-dept">Dept: ${user.role}</div>
            </div>
        `;
    } else {
        const bg = cp.idCardBackBase64 ? `background-image: url('${cp.idCardBackBase64}');` : '';
        return `
            <div class="id-card-wrapper back-side" style="${bg}">
                <div class="id-back-details">
                    <table>
                        <tr><td>Guardian Name</td><td>${user.fatherName || 'N/A'}</td></tr>
                        <tr><td>CNIC No</td><td>${user.cnic || 'N/A'}</td></tr>
                        <tr><td>Emergency No</td><td>${user.emergencyContact || 'N/A'}</td></tr>
                        <tr><td>Blood Group</td><td>${user.bloodGroup || 'N/A'}</td></tr>
                        <tr><td>Date of Joining</td><td>${user.startDate || 'N/A'}</td></tr>
                    </table>
                </div>
            </div>
        `;
...
window.renderEmployeeCardsTab = function() {
    const db = getDb();
    const container = document.getElementById('admin-tab-cards-container');
    if (!container) return;
    ...
    if (activeUsers.length === 0) {
        container.innerHTML = `<div class="empty-state" style="grid-column: 1 / -1;"><h3>No Active Employees</h3><p>Add employees to generate ID cards.</p></div>`;
        return;
    }

    activeUsers.forEach(user => {
        const frontHTML = window.generateIdCardHTML(user, 'front');
        
        const cardBox = document.createElement('div');
        cardBox.className = 'section-card bg-glass';
        cardBox.style.display = 'flex';
        cardBox.style.flexDirection = 'column';
        cardBox.style.alignItems = 'center';
        cardBox.style.padding = '1.5rem';
        
        cardBox.innerHTML = `
            ${frontHTML}
            <div style="margin-top: 1rem; width: 100%; display: flex; gap: 0.5rem;">
                <button class="btn btn-outline" style="flex: 1;" onclick="viewSingleIdCard('${user.id}')"><i class="fa-regular fa-eye"></i> View & Print</button>
            </div>
        `;
...
window.viewSingleIdCard = function(userId) {
    const db = getDb();
    const user = db.users.find(u => u.id === userId);
    if (!user) return;
    
    document.getElementById('single-card-front-container').innerHTML = window.generateIdCardHTML(user, 'front');
    document.getElementById('single-card-back-container').innerHTML = window.generateIdCardHTML(user, 'back');
    
    document.getElementById('btn-print-single-card').setAttribute('data-userid', userId);
    
    openModal('modal-view-id-card');
};

window.printSingleIdCard = function() {
    const userId = document.getElementById('btn-print-single-card').getAttribute('data-userid');
    if (!userId) return;
    ...
    const printContainer = document.getElementById('id-card-print-container');
    printContainer.innerHTML = '';
    
    printContainer.innerHTML += window.generateIdCardHTML(user, 'front');
    printContainer.innerHTML += window.generateIdCardHTML(user, 'back');
    
    closeModal('modal-view-id-card');
    
    document.querySelectorAll('.tab-view').forEach(t => t.classList.remove('print-active'));
    document.getElementById('admin-tab-employees').classList.add('print-active');
...
window.printAllIdCards = function() {
    const db = getDb();
    const activeUsers = db.users.filter(u => u.status === 'Active');
    
    if (activeUsers.length === 0) {
        showToast("Error", "No active employees to print.", "error");
        return;
    }
